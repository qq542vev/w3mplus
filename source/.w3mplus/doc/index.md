...
[`gf`]{#key-gf}
: レンダリングビューとソースビューを切り替えます。

[`gF`]{#key-gF}
: 現在のドキュメントのソースを外部テキストエディタで開く。環境変数 `EDITOR` でテキストエディタを指定できます。
...
クイックマークは、よく訪れるページに素早くアクセスするための、無駄を省いたブックマークです。クイックマークは、アルファベットや数字に URL を割り当てただけのものです。そのため、わずか3回のキー操作で保存したり開いたりすることができます。クイックマークはブラウザのセッションをまたいで保持されます。
...
| [`z.`](#key-z.2E) | ドキュメントを半ページ左にスクロールする。 | |
| [`z.`](#key-z.2E) | ドキュメントを半ページ右にスクロールする。 | |
| [`z.`](#key-z.2E) | 画像の表示倍率をリセットする。 | |
| [`z.`](#key-z.2E) | 現在の行を最下部にスクロールする。 | |
| [`z.`](#key-z.2E) | ドキュメントを1列左にスクロールする。 | `LEFT` |
| [`z.`](#key-z.2E) | ドキュメントを1列右にスクロールする。 | `RIGHT` |
| [`z.`](#key-z.2E) | 現在の行を最上部にスクロールする。 | |
| [`z.`](#key-z.2E) | 現在の行を中央にスクロールする。 | `CENTER_V` |
